...
public class FishMovement : MonoBehaviour
{
    ...
    public bool appearFromHouse = false;

    bool comeBackInHouse = false;
    ...
    void Start() {
        ...
        Vector3 angle = this.transform.localEulerAngles * Mathf.Deg2Rad;
        this.directionX = Mathf.Sin(angle.y);
        this.directionZ = Mathf.Cos(angle.y);
        ...
        transform.eulerAngles = new Vector3(transform.eulerAngles.x, getFishOrientationDegree(), transform.eulerAngles.z);
    ...
    void ascendingUpdate(){
        ...
        transform.position += new Vector3(directionX, 1, directionZ) * this.speed;
        if(transform.position.y >= ground.transform.position.y + 0.055f)
        {
            this.appearFromHouse = false;
            
        }   
    }
    void descendingUpdate()
    {
        Vector3 dir = this.rock.transform.position - transform.position - new Vector3(0, 0.01f, 0.0f);
        dir.Normalize();
        //transform.eulerAngles = new Vector3(transform.eulerAngles.x, getFishOrientationDegree(), transform.eulerAngles.z);
        transform.position += dir * this.initialSpeed;

        directionX = (directionX > newDirX) ? Mathf.Max(newDirX, directionX - rotationSpeed*2) : Mathf.Min(newDirX, directionX + rotationSpeed * 2);
        directionZ = (directionZ > newDirZ) ? Mathf.Max(newDirZ, directionZ - rotationSpeed*2) : Mathf.Min(newDirZ, directionZ + rotationSpeed * 2);
        transform.eulerAngles = new Vector3(transform.eulerAngles.x, getFishOrientationDegree(), transform.eulerAngles.z);


        //transform.position += new Vector3(directionX, 1, directionZ) * this.speed;
        float distToRock = Vector3.Distance(this.rock.transform.position, transform.position);
        if (distToRock < 0.01)
        {
    ...
    void normalUpdate(){
        ...
        transform.position += new Vector3(Mathf.Clamp(directionX * this.speed, wallLeft.transform.position.x, wallRight.transform.position.x), 0, Mathf.Clamp( directionZ * this.speed, wallDown.transform.position.z, wallUp.transform.position.z)) ;
        transform.eulerAngles = new Vector3(transform.eulerAngles.x, getFishOrientationDegree(), transform.eulerAngles.z);


        if (!comeBackInHouse && !this.isFaster && !this.isEating && !this.isInsideCliff)
        {
            float distToRock = Vector3.Distance(this.rock.transform.position, transform.position);

            Vector3 dir = this.rock.transform.position - transform.position;
            float angleFishRock = Mathf.Atan2(dir.x, dir.z);

            angleFishRock = (angleFishRock < 0) ? (angleFishRock + Mathf.PI * 2) : (angleFishRock % (Mathf.PI * 2));


            if(Random.Range(0,100)==1 && distToRock > 0.25 && ((angleFishRock<2.3 && angleFishRock >2 ) || (angleFishRock > 4.5 && angleFishRock < 5)) && GameObject.FindGameObjectsWithTag("fish").Length > 5){
                if (fishNumber() > 5)
                {
                   // this.comeBackInHouse = true;
                    //this.newDirZ = Mathf.Cos(angleFishRock);
                    //this.newDirX = Mathf.Sin(angleFishRock);
                }
            }
    ...
    {
        return collideFurther(dir, 0.065f);
    }
